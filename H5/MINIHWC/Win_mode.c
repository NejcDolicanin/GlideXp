...
setVideoMode( HWND hwnd, int xRes, int yRes, int  pixelSize, int refresh, void *hmon ) 
{
	...
	GDBG_INFO (80, "Display monitor device: %s\n", monInfo.szDevice);
	...
	*/
	GDBG_INFO (80, "Display is currently %dx%dx%dbpp\n", devMode.dmPelsWidth, devMode.dmPelsHeight,
...
#define FN_NAME "checkResEMCallback"
	...
	res++) {
		if (
			...
			for (ref = 0; ref < (sizeof(RefreshRates) / sizeof(FxU32)); ref++) {
				if (surfaceDesc->dwRefreshRate == 0 || surfaceDesc->dwRefreshRate == RefreshRates[ref])
