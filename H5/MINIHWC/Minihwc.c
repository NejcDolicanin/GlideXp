...
static LRESULT APIENTRY _XPAltTabProc(
									  HWND hwnd, 
									  UINT uMsg, 
									  WPARAM wParam, 
									  LPARAM lParam);
			...
			*/
			if (handle != NULL) {
				...
				if (1) {
					...
					if (GetMonitorInfoA(handle, (LPMONITORINFO) &monInfo)) {
...
#define FN_NAME "hwcInit"
	{
		...
		for (monitor = 0; monitor < num_monitor; monitor++) {
			...
			// For 4 Chip boards to 1 chip in NT
			if (hInfo.boardInfo[monitor].osNT && hInfo.boardInfo[monitor].pciInfo.numChips == 4) 
	...
	
	{
		...
		if (bInfo->pciInfo.numChips > 1)
		{
			...
			for (chip = 1 ; chip < bInfo->pciInfo.numChips ; chip++)
			{
				...
				/* Get back 4 register space mappings for each chip */
				for (i = 0 ;  i < 4 ; i++)
...
#define FN_NAME "hwcAllocBuffers"
	...
	if (nAuxBuffers > 0) 
	{
		bInfo->buffInfo.auxBuffEnd0  = bInfo->buffInfo.auxBuffStart0 = freeMem;
	...
	if (bInfo->buffInfo.enable2ndbuffer)
	{
		if (nAuxBuffers > 0) 
		{
			bInfo->buffInfo.auxBuffEnd1 = bInfo->buffInfo.auxBuffStart1 = freeMem;
...
#define FN_NAME "hwcGetSurfaceInfo"
	...
	LPDIRECTDRAWSURFACE2 surf = (LPDIRECTDRAWSURFACE2) sfc;
	DDSURFACEDESC desc;
	DWORD ddErr;
	FxBool retVal;
	FxU32 lfbBase,                    /* linear addr for base of LFB */
		  lpSurface;                  /* Linear addr for the surface */
	
	/* Lock the surface and get some info */
	desc.dwSize = sizeof(desc);
	ddErr = IDirectDrawSurface2_Lock(surf, 0, &desc, DDLOCK_WAIT | DDLOCK_SURFACEMEMORYPTR, 0);
	retVal = (ddErr == DD_OK);
	if (!retVal) 
	{
...
FxU32 hwcAllocWinContext(hwcBoardInfo* bInfo)
{
...
FxBool hwcFreeWinContext(hwcBoardInfo* bInfo,
                  FxU32         winContextId)
{
...
FxBool hwcAllocWinFifo(hwcBoardInfo* bInfo, 
                HwcWinFifo*   fifo,
                FxU32*        surface)
{
#define FN_NAME "hwcAllocWinFifo"
	...
	if (retVal)
	{
		LPDIRECTDRAW objDD = NULL;
		LPDIRECTDRAWSURFACE objSentinal = NULL,
							objFifo = NULL;
		DDSURFACEDESC fifoDesc, sentinalDesc;
		DWORD surfaceWidth;
		...
		{
			LPDIRECTDRAWSURFACE2 objSurface = NULL;
...
__errSurfaceDDObj:
		...
		fifoDesc.ddsCaps.dwCaps = (DDSCAPS_OFFSCREENPLAIN | DDSCAPS_VIDEOMEMORY);
		...
		fifoDesc.ddsCaps.dwCaps = (DDSCAPS_OFFSCREENPLAIN | DDSCAPS_VIDEOMEMORY);
		fifoDesc.dwFlags = (DDSD_CAPS | DDSD_WIDTH | DDSD_HEIGHT | DDSD_PITCH | DDSD_PIXELFORMAT);
		...
		retVal = (IDirectDraw_CreateSurface(objDD, &sentinalDesc, &objSentinal, 0) == DD_OK);
...
#define HWC_DIRECTDRAW_DRIVER_NOSYSLOCK_FLAGS 0 
...

		{
			...
			const FxU32 lfbBase = bInfo->linearInfo.linearAddress[1]; /* lfb space */
			FxU32 tileMark;      
...
#define FN_NAME "hwcInitVideo"
	...
	
	{
		...
		if (bInfo->vidInfo.vRefresh > GR_REFRESH_120Hz)
			...
			// Check Global Refresh Override
			GDBG_INFO(80, "%s:  checking for refresh override %s\n", FN_NAME, "FX_GLIDE_REFRESH");
			if (GETENV("FX_GLIDE_REFRESH")) {
				refresh = atoi(GETENV("FX_GLIDE_REFRESH"));
				GDBG_INFO(80, "%s:  refresh rate set to %d\n", FN_NAME, refresh);
			}
			
			{
				char ref_str[64];
				// First one is for general resolution
				_snprintf (ref_str, 64, "GXP_GLIDE_REFRESH_%dx%d", bInfo->vidInfo.xRes, bInfo->vidInfo.yRes);
				GDBG_INFO(80, "%s:  checking for refresh override %s\n", FN_NAME, ref_str);
				if (GETENV(ref_str)) {
					refresh = atoi(GETENV(ref_str));
					GDBG_INFO(80, "%s:  refresh rate set to %d\n", FN_NAME, refresh);
				}
				
				// This on is bits per pixel dependant too
				_snprintf (ref_str, 64, "GXP_GLIDE_REFRESH_%dx%dx%d", bInfo->vidInfo.xRes, bInfo->vidInfo.yRes, bInfo->h3pixelSize*8);
				GDBG_INFO(80, "%s:  checking for refresh override %s\n", FN_NAME, ref_str);
				if (GETENV(ref_str)) {
					refresh = atoi(GETENV(ref_str));
					GDBG_INFO(80, "%s:  refresh rate set to %d\n", FN_NAME, refresh);
...
#define BINFO_VIDINFO_YRES	(bInfo->vidInfo.yRes)
  ...
  */
  ...
  GDBG_INFO(120, FN_NAME ":  returning\n");
  return FXTRUE;
...
#define FN_NAME "hwcGammaTable"
	...
	if (bInfo->pciInfo.numChips == 4 && bInfo->h3pixelSample >= 4) rs = 1;
	
	/* Load the table into the Display driver as above */
	for (i = 0; i < nEntries; i++) {
		gRamp[i] =
			(((r[i] & 0xff)>>rs) << RED_SHIFT) |
			(((g[i] & 0xff)>>rs) << GREEN_SHIFT) |  
			(((b[i] & 0xff)>>rs) << BLUE_SHIFT);
			gss_red[i] = r[i];
			gss_green[i] = g[i];
			gss_blue[i] = b[i];
	...
	/* Hack for V5 6000 4x and 8x FSAA*/
	...
	*/
		...
		*/
		...
		index++)
		{
			theRamp[index*3]   = (WORD) (r[index] | (r[index] << 8));
			theRamp[index*3+1] = (WORD) (b[index] | (g[index] << 8));
			theRamp[index*3+2] = (WORD) (g[index] | (b[index] << 8));
		}
	...
	/*
	   ...
	   * version is WinNT-friendly.
		*/
		...
		index++)
		{
			r[index] = (theRamp[index*3] >> 8);
			g[index] = (theRamp[index*3+1] >> 8 );
			b[index] = (theRamp[index*3+2] >> 8 );
		}
	}
#endif
	/* Hack for V5 6000 4x and 8x FSAA*/
...
char * hwcGetenv(char *a) 
{
#if __WIN32__
	extern char *getRegPath();
	char *retVal = NULL;
...
#define FN_NAME "hwcShareContextData"
	...
	if( HWCEXT_PROTOCOL( bInfo->boardNum ) )
	{
		if (bInfo->osNT) {
			...
			retVal = ExtEscape((HDC) bInfo->hdc, HWCEXT_ESCAPE(bInfo->boardNum), sizeof(ctxReq), (void *) &ctxReq,
                sizeof(ctxRes), (void *) &ctxRes);
			
			// Have to do this because the context isn't working properly in Windows XP for me
			if (ctxRes.optData.contextDwordNTRes.dwordOffset == 0) {
				cLostPointer = *data = &dummyContextDWORD;
				
				// We'll chuck a real mental if the #define isn't set
...
#endif
  bInfo->lostContextDWORD = (FxU32) *data;
