...
**
** $Header: f:\\cvsroot/Glide3x/h5/glide3/src/gsfc.c,v 1.6 2002/05/04 02:07:43 johndoe Exp $
** $Log: 
  ...
  if ( type != GR_SURFACECONTEXT_WINDOWED ) {
    ...
    for( i = 0; i < MAX_NUM_CONTEXTS; i++ ) {
      if ( _GlideRoot.surfaceGCHeap[i] == 0 ) {
        ...
        hwcShareContextData(ctx->bInfo, &(ctx->lostContext));
...
#define FN_NAME "grSurfaceReleaseContext"
  ...
  for( i = 0; i < MAX_NUM_CONTEXTS; i++ ) 
  {
	...
	{
      GrGC* gc = (GrGC*)ctx;
      const struct cmdTransportInfo* fifoInfo = &gc->cmdTransportInfo;
...
#define FN_NAME "grSurfaceSetRenderingSurface"
  ...
  gc->lfbBuffers[0] = gc->lfbBuffers[1] = sfcInfo.lpLFB;
  gc->bInfo->buffInfo.bufLfbStride = sfcInfo.fbStride;
