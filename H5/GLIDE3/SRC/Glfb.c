...
**
** $Header: /cvsroot/glide/glide3x/h5/glide3/src/glfb.c,v 1.7 2000/11/17 21:31:05 joseph Exp $
** $Log: 
...
#define FN_NAME "_grLfbWriteRegion"
  ...
  if (_grLfbLock(GR_LFB_WRITE_ONLY_EXPLICIT_EXT, 
	...
	FxI32 info_strideInBytes;
	void  *info_lfbPtr;

	// Now we need to really fudge with things
	// Set info.lfbPtr data to the start of the 'last' line
	// then screw with the stride
	info_lfbPtr = info.lfbPtr;//((char*)info.lfbPtr) + (gc->state.screen_height-1)*info.strideInBytes;
	info_strideInBytes = info.strideInBytes;// * -1;
    
    
    srcData = (FxU32 *) src_data;
    dstData = (FxU32 *) (((char*)info_lfbPtr)+ (dst_y*info_strideInBytes));
    scanline = src_height;
    ...
    case GR_LFB_SRC_FMT_ZA16:
      ...
      dstJump = info_strideInBytes - length;
      if (aligned) {
        while(scanline--) {
