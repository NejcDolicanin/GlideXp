...
**
** $Header: /cvsroot/glide/glide3x/h5/glide3/src/gtexdl.c,v 1.3 2000/11/15 23:32:53 joseph Exp $
** $Log: 
...
// Handling the stupid TMU0 bug
  ...
  if (tmu == GR_TMU1)
  {
	  int i;
	  for (i = start; i < end; i++) 
		  gc->state.tmu1Palette.data[i] = pal->data[i];

	  gc->state.tmu1PaletteSet = FXTRUE;
  }
  else if (gc->state.tmu1PaletteSet)
  {
	// Just update the entire palette
	  _grTexDownloadPalette(GR_TMU1, type, &gc->state.tmu1Palette, start, end);
  }
#endif
#endif
...
{
	grTexDownloadTableExt(tmu, type, data);
} /* grTexDownloadTable */
#endif
...
_grTexDownloadMipMapLevelPartialTiled(GrChipID_t tmu, 
                                      ...
                                      int t, int maxT, int minS, int maxS)
{
#define FN_NAME "_grTexDownloadMipMapLevelPartialTiled"
...

  {
    ...
    /* Minimum maxS is 4 for DXT1 format. */
    else if (format == format == GR_TEXFMT_ARGB_CMP_DXT1)
	{
	  minS = 0;
      if (!maxS) maxS = WIDTH_BY_ASPECT_LOD_DXT(aspectRatio, thisLod);
	  else if (maxS < 8) maxS = 4;
	}
    /* Minimum maxS is 4 for DXT2, DXT3, DXT4 and DXT5 format. */
...
// Ugly Hack Time
#ifdef GLIDE2
#undef grTexDownloadMipMapLevelPartial
#endif
...
#ifdef GLIDE2
{
	return _G3grTexDownloadMipMapLevelPartial (tmu, startAddress,
		AS_GLIDE3_LOD(thisLod), AS_GLIDE3_LOD(largeLod),
		AS_GLIDE3_ASPECT(aspectRatio),format, evenOdd, data, t, max_t);
}

#define grTexDownloadMipMapLevelPartial _G3grTexDownloadMipMapLevelPartial

FxBool _G3grTexDownloadMipMapLevelPartial (GrChipID_t tmu, FxU32 startAddress,
          GrLOD_t thisLod, GrLOD_t largeLod, GrAspectRatio_t aspectRatio,
          GrTextureFormat_t format, FxU32 evenOdd, 
          void *data, 
          int t, int max_t)
    ...
    
  {
    ...
    if (memInfo->texTiled) {
      ...
      /* Do the download */
      {
        ...
        FxU32 width, formatSel, widthSel, max_s;

        if (format == GR_TEXFMT_ARGB_CMP_FXT1)
          width =_grMipMapHostWHCmp4Bit[G3_ASPECT_TRANSLATE(aspectRatio)][thisLod][0];
        else if (format == GR_TEXFMT_ARGB_CMP_DXT1)
		{
          width = _grMipMapHostWHDXT[G3_ASPECT_TRANSLATE(aspectRatio)][thisLod][0];
		  //FxU32 width2 = _grMipMapHostWHDXT[G3_ASPECT_TRANSLATE(aspectRatio)][thisLod][0];
		  /* DXT1 gives us headaches, so we need to do a 'hack' */
		}
        else if (format >= GR_TEXFMT_ARGB_CMP_DXT2 && format <= GR_TEXFMT_ARGB_CMP_DXT5)
          width =_grMipMapHostWHDXT[G3_ASPECT_TRANSLATE(aspectRatio)][thisLod][0];
        else
...
/*-------------------------------------------------------------------
  ...
  Arguments:
    ...
    data         - pointer to mipmap data
	t            - row to download
	s            - first pixel to download
	max_s        - last pixel on row to download
  Return:
    none
  -------------------------------------------------------------------*/
