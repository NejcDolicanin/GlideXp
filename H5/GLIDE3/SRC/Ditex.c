...
**
** $Header: /cvsroot/glide/glide3x/h5/glide3/src/ditex.c,v 1.3 2000/11/15 23:32:52 joseph Exp $
** $Log: 
 ...
 */
const FxU32 _grMipMapHostSizeDXT[7][GR_LOD_LOG2_2048+1] =
{
  {                  /* 8:1 aspect ratio */
         16,         /*  0 :    4x4    */
         16,         /*  1 :    4x4    */
...
static const FxU32 _grMipMapSizeDXT[7][16] = {
  {                  /* 8:1 aspect ratio */
    0x000010,        /*   0 :    4x4    */
    0x000010,        /*   1 :    4x4    */
    0x000010,        /*   2 :    4x4    */
 ...
 * textures, where 4x4 is the minimum size. DXT2,3,4,5
 */
const FxI32 _grMipMapOffsetDXT[7][16] = {
  {                  /* 8:1 aspect ratio */
 ...
 * textures, where 4x4 is the minimum size. DXT2,3,4,5
 */
const FxI32 _grMipMapOffset_TsplitDXT[7][16] = {
  {                  /* 8:1 aspect ratio */
...
#define FN_NAME "_grTexTextureMemRequired"
  ...
  // Force texture compression on napalm
//  if (IS_NAPALM(gc->bInfo->pciInfo.deviceID) && _GlideRoot.environment.compressTextures	== 2)
//  {
//	format = GR_TEXFMT_ARGB_CMP_FXT1;
//  }
  ...
  case GR_TEXFMT_ARGB_CMP_DXT5:
    ...
    if ( evenOdd == GR_MIPMAPLEVELMASK_BOTH ) {
      memrequired  = _grMipMapOffsetDXT[G3_ASPECT_TRANSLATE(aspect)]
        [small_lod];
      memrequired -= _grMipMapOffsetDXT[G3_ASPECT_TRANSLATE(aspect)]
        [large_lod+1];
    } else { 
      ...
      while (large_lod >= small_lod) {    /* sum up all the mipmap levels */
        if ((large_lod ^ evenOdd) & 1)    /* that match the XOR mask      */
          memrequired += _grMipMapSizeDXT[G3_ASPECT_TRANSLATE(aspect)]
            [large_lod];
        large_lod--;
      }
    }
...
#define FN_NAME "_grTexCalcBaseAddress"
  ...
  // Force texture compression on napalm
//  if (IS_NAPALM(gc->bInfo->pciInfo.deviceID) && _GlideRoot.environment.compressTextures)
//  {
//	format = GR_TEXFMT_ARGB_CMP_FXT1;
//  }
  ...
  case GR_TEXFMT_ARGB_CMP_DXT4:
  case GR_TEXFMT_ARGB_CMP_DXT5:
    /* DXT2,3,4,5 format: Don't mirror the aspect ratios, because of the 4x4 limit */
    if ( odd_even_mask == GR_MIPMAPLEVELMASK_BOTH ) {
      sum_of_lod_sizes = _grMipMapOffsetDXT[aspect]
        [large_lod + 1];
    } else {
      if (((odd_even_mask == GR_MIPMAPLEVELMASK_EVEN) && (large_lod & 1)) ||
          ((odd_even_mask == GR_MIPMAPLEVELMASK_ODD) && !(large_lod & 1)))
      ...
      sum_of_lod_sizes =
        _grMipMapOffset_TsplitDXT[aspect][large_lod];
    }
    break;
...
#ifdef GLIDE2
{
	return _G3grTexCalcMemRequired(AS_GLIDE3_LOD(small_lod), 
		AS_GLIDE3_LOD(large_lod), AS_GLIDE3_ASPECT(aspect), format);
}
#define grTexCalcMemRequired _G3grTexCalcMemRequired
FxU32 _G3grTexCalcMemRequired( GrLOD_t small_lod, GrLOD_t large_lod, 
            GrAspectRatio_t aspect, GrTextureFormat_t format )
#endif
{
  FxU32 memrequired;
...
#ifdef GLIDE2
{
  ...
  memrequired = _grTexTextureMemRequired( AS_GLIDE3_LOD(info->smallLodLog2),
                                          AS_GLIDE3_LOD(info->largeLodLog2),
                                          AS_GLIDE3_ASPECT(info->aspectRatioLog2),
                                          info->format,
                                          evenOdd,
                                          FXTRUE );
...
#ifdef GLIDE2
{
	...
	_G3grTexDownloadMipMap ( tmu, startAddress, evenOdd, info );
...
void _G3grTexDownloadMipMap ( GrChipID_t tmu, FxU32 startAddress, FxU32
            evenOdd, GrTexInfo  *info )
#endif
{
...
GR_DIENTRY(grTexDownloadTablePartial, void,
           ( GrChipID_t tmu, GrTexTable_t type, 
            void *data, int start, int end ))
{
	grTexDownloadTablePartialExt(tmu, type, data, start, end);
} /* grTexDownloadTablePartial */
...
// Ugly Hack Time
#ifdef GLIDE2
#undef grTexDownloadMipMapLevel
#endif
...
#ifdef GLIDE2
{
	_G3grTexDownloadMipMapLevel (tmu, startAddress, AS_GLIDE3_LOD(thisLod),
		AS_GLIDE3_LOD(largeLod), AS_GLIDE3_ASPECT(aspectRatio),
		format, evenOdd, data);
}
#define grTexDownloadMipMapLevel _G3grTexDownloadMipMapLevel

void _G3grTexDownloadMipMapLevel( GrChipID_t tmu, FxU32 startAddress, GrLOD_t thisLod,
            GrLOD_t largeLod, GrAspectRatio_t aspectRatio,
            GrTextureFormat_t format, FxU32 evenOdd, void *data )
#endif
