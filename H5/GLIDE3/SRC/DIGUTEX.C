                    ...
                    ))
{
  FxU32
    ...
    tLod = 0,
    texturemode = 0,
    filterMode = 0,                 /* filter mode bits */
    clampMode = 0;                  /* clamp mode bits */
  ...
  info.smallLodLog2 = AS_GLIDE3_LOD(small_lod);
  info.largeLodLog2 = AS_GLIDE3_LOD(large_lod);
  info.aspectRatioLog2 = AS_GLIDE3_ASPECT(aspect_ratio);
  info.format = format;
   ...
   */
  // Hack in Glide 3 stuff here
  

  baseAddress = _grTexCalcBaseAddress( gc->tmu_state[tmu].freemem_base, 
                                       AS_GLIDE3_LOD(large_lod),
                                       AS_GLIDE3_ASPECT(aspect_ratio) ,
                                       format, 
                                       odd_even_mask );
  ...
  gc->mm_table.data[mmid].float_lod_bias = lod_bias;
...
//  const  GrMipMapInfo *mminfo;
  ...
  // Make all this shit glide 3 compatable
  TO_GLIDE3_LOD(mminfo->lod_max);
  TO_GLIDE3_LOD(lod);
  TO_GLIDE3_ASPECT(mminfo->aspect_ratio);
  ...
  *src_base = (void *) (((FxU32)*src_base) +
	  ((_grMipMapHostSize[mminfo->aspect_ratio][lod]
		* _grBitsPerTexel[mminfo->format])>>3));

  // Put them all back to their Glide 2 values
  TO_GLIDE2_LOD(mminfo->lod_max);
  //TO_GLIDE2_LOD(lod);
  TO_GLIDE2_ASPECT(mminfo->aspect_ratio);
