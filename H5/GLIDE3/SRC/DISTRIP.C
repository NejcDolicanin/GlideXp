...
**
** $Header: /cvsroot/glide/glide3x/h5/glide3/src/distrip.c,v 1.3 2000/11/15 23:32:52 joseph Exp $
** $Log: 
...
GR_ENTRY(grDrawPolygon, void, ( int nVerts, int ilist[], GrVertex vlist[] ))
{
#define FN_NAME "grDrawPolygon"
  ...
  while (nVerts >= 3) {
    grDrawTriangle(vlist+*(indices), vlist+*(indices+1), vlist+*(indices+2));
    indices+=3;
    nVerts-= 3;
  }
...
GR_ENTRY(grDrawPlanarPolygon, void, ( int nVerts, int ilist[], GrVertex vlist[] ))
{
  grDrawPolygon (nVerts, ilist, vlist);
}

GR_ENTRY(grDrawPlanarPolygonVertexList, void, ( int nVerts, GrVertex vlist[] ))
{
  grDrawVertexArrayContiguous(GR_POLYGON, nVerts, vlist, sizeof(GrVertex));
}

GR_ENTRY(grDrawPolygonVertexList, void, ( int nVerts, GrVertex vlist[] ))
{
  grDrawVertexArrayContiguous(GR_POLYGON, nVerts, vlist, sizeof(GrVertex));
}
