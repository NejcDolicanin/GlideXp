...
{
#ifdef GLIDE2
#define DEPTH_VALUE 0xFFFFU
#else
#define DEPTH_VALUE 0xFFFFFFFFUL
#endif
...
#ifdef GLIDE2
    if (gc->pluginInfo.moduleHandle == NULL) gc->pluginInfo.moduleHandle = LoadLibrary("3dfxspl2.dll");
#else
...
#ifdef GLIDE2
  for (t = 0; t < GLIDE_NUM_TMU; t ++) {
    gc->state.current_mm[t] = GR_NULL_MIPMAP_HANDLE;
  }

  gc->state.paramHints = GR_STWHINT_ST_DIFF_TMU0;
  memset (gc->state.tmu1Palette.data, 0, sizeof(gc->state.tmu1Palette));
  gc->state.tmu1PaletteSet = FXFALSE;
#endif
...
GR_ENTRY(grSstWinOpen, GrContext_t, ( FxU32                   hWnd,
                                      ...
                                      int                     nAuxBuffers) )
#else
#define WINOPEN_RETURN(a) return ((a) != 0)
...
#define MIN_FIFO_SIZE     0x10000
...
#ifndef		__linux__
  /* Doing this should be ok. It will allow us to run programs that didn't supply a
   * window, but really do have one */
  if (!hWnd)
    hWnd = (FxU32) GetActiveWindow();
  if (!hWnd)
    GrErrorCallback("grSstWinOpen: need to use a valid window handle",
                    FXTRUE);
    //MessageBox ((HWND) hWnd, "Attach debug here", "Waiting", MB_OK);
#endif	/* defined(__linux__) */
  ...
  
  {
    ...
    if (IS_NAPALM(gc->bInfo->pciInfo.deviceID)) {
      ...
      WINOPEN_RETURN ( grSstWinOpenExt(hWnd,
                               resolution, 
                               refresh, 
                               format, 
                               origin, 
...
#define MIN_FIFO_SIZE     0x10000
  ...
  
  {
    ...
    */
	...
	/* Can Only use 1 chip in NT */
//	if (gc->bInfo->osNT && gc->chipCount == 4) 
//		gc->chipCount = 1;
	...
	{
		case 4:
			if (gc->grPixelSample > 8) gc->grPixelSample = 8;
		...
		case 2: 
			if (gc->grPixelSample > 4) gc->grPixelSample = 4;
			switch( gc->grPixelSample )
			{
				case 4:
						gc->sliCount 			= 1;
		...
		case 1:
			if (gc->grPixelSample > 2) gc->grPixelSample = 2;
			switch( gc->grPixelSample )
			{
				case 2:
      					gc->sliCount 			= 1; 
...
//adjust offset index for 4 chip cards
    ...
    */
    ...
    /* Force analog for 4 chip cards when doing FSAA */
    /* Seems to work fine when not doing any FSAA */
    if( gc->chipCount == 4 && gc->sliCount != 4) 
        gc->bInfo->h3analogSli = 1 ;
    else
        gc->bInfo->h3analogSli = 0 ;
...
#ifdef GLIDE2
#undef grSstWinClose 
GR_ENTRY(grSstWinClose, FxBool, (void))
{
  GR_DCL_GC;
  return _G3grSstWinClose((GrContext_t) gc);
}
...
#ifdef GLIDE2
GR_DIENTRY(grSstResetPerfStats, void, (void) )
{
...
#ifdef GLIDE2
GR_DIENTRY(grSstStatus, FxU32, (void) )
{
  return _grSstStatus();
}
#endif
...
GR_ENTRY(grGammaCorrectionValue, void, (float gamma))
{
  ...
  guGammaCorrectionRGB(gamma, gamma, gamma);
       GR_END();
...
GR_ENTRY(grSstControlMode, FxBool, (GrControl_t code)) 
{
