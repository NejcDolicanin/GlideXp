...
//  FxU32               baseAddress;
  ...
  // Setup the info
  info.smallLodLog2 = AS_GLIDE3_LOD(mminfo->lod_min);
  info.largeLodLog2 = AS_GLIDE3_LOD(mminfo->lod_max);
  info.aspectRatioLog2 = AS_GLIDE3_ASPECT(mminfo->aspect_ratio);
  ...
  if ((mminfo->format == GR_TEXFMT_YIQ_422) ||
    ...
    */
    if (table == 0)
	grTexNCCTable( tmu, GR_NCCTABLE_NCC0 );
    else
	grTexNCCTable( tmu, GR_NCCTABLE_NCC1 );
  } /* if it's an NCC texture */


  // Update Registers here
  // FIXME - This will cause a speed penalty doing things this way

  // Set MipMap Mode
  grTexMipMapMode (tmu, mminfo->mipmap_mode, mminfo->trilinear);

  // Set the filter mode
  grTexFilterMode (tmu, mminfo->minfilter_mode, mminfo->magfilter_mode);

  // Set the Clamp mode
  grTexClampMode(tmu, mminfo->s_clamp_mode, mminfo->t_clamp_mode);

  // Set LOD Bias
  grTexLodBiasValue(tmu, mminfo->float_lod_bias );
