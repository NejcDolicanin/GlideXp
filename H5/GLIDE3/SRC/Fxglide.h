...
/*--------------------------------------------------------------------------
  State Restoration Buffer for TMUs
  --------------------------------------------------------------------------*/
struct tmuState_s {
  ...
  */
  ...
  struct{
    GrCoordinateSpaceMode_t coordinate_space_mode;
	  ...
	  GrSTWHint_t paramHints;
	  GuTexPalette tmu1Palette; // TMU1's palette
	  FxBool tmu1PaletteSet;
#endif
  FxU32 forced32BPP;
} GrState;
...
#define kPackMaskB  0x00FFUL
  ...
  struct {                      /* environment data             */
    ...
    FxBool texSubLodDither;     /* Always do Subsample lod-dithering */
    FxU32  texSubToggleKey;     /* Raw Key code for texSub toggle */
    ...
    FxU32  ogToggleKey;         /* Raw Key code for Ordered Grid AA toggle */
    FxU32  aaScreenshotKey;     /* Raw Key code for Screenshot taking */
    ...
    FxU32  compressTextures;    /* force fxt1 texture compression */
...
#define TRISETUP (*gc->triSetupProc)
...
//#define HWCQUERYCONTEXTXP() if (hwcQueryContextXP(gc->bInfo)) return;
//#define HWCQUERYCONTEXTXP_RET() if (hwcQueryContextXP(gc->bInfo)) return 0;
