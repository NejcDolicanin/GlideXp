...
**
** 
** $Header: /cvsroot/glide/glide3x/h5/glide3/src/gglide.c,v 1.7 2000/11/24 18:36:47 alanh Exp $
** $Log: 
**  52   GlideXP   1.42.1      12/12/01 Ryan Nunn       grTBufferWriteMaskExt was
**       causing problems on 4 chip boards, so I disabled it for now.
**
** $OldLog
...
#else
		vertex[0].x = -1.0f -(float)gc->state.screen_width ;
		vertex[0].y = -1.0f;
		vertex[0].depth = (float)depth;
...
#define FN_NAME "grBufferClear"
	...
	
	{
		...
		if (!doneP) {
			...
			/* This code can only possibly help in Sli */
			if (gc->sliCount > 1)      
			{
...
#if USE_GR_TRI_FILL_ON_NAPALM
			else if (IS_NAPALM(gc->bInfo->pciInfo.deviceID))
			{
				/* */
				/* Not on an even sli bandheight boundary or range... */
				/* so use a triangle fill (yuck!) */
...
#endif
			else if (!gc->bInfo->sdRAM && !gc->windowed) {
...
#define FN_NAME "grBufferSwap"
	...
	GDBG_INFO_MORE(gc->myLevel,FN_NAME ": (%d)\n",swapInterval);
...
#define FN_NAME "grGlideSetState"
	...
	/* Glide2 state hack */
#ifdef GLIDE2
	state = _G2GetStatePointer(state);
#endif
...
#define FN_NAME "_grUpdateParamIndex"
	...
	**        is enabled
	*/
#if defined(GLIDE3) && (GLIDE3_ALPHA) && !defined(GLIDE2)
