 ...
 **
 ** $Header: /cvsroot/glide/glide3x/h5/glide3/src/gdraw.c,v 1.3 2000/11/15 23:32:53 joseph Exp $
 ** $Log: 
    ...
    while (count > 0) {
      ...
      for (k = 0; k < vcount; k++) {
        ...
        
        {
          ...
          gc->pool.ftemp1 = floor(FARRAY(vPtr, gc->state.vData.vertexInfo.offset));// + bias;
          gc->pool.ftemp2 = floor(FARRAY(vPtr, gc->state.vData.vertexInfo.offset + 4) + 0.5);// + bias;
          ...
          x = gc->pool.ftemp1;//((*(FxU32*)&gc->pool.ftemp1 & (0xFFFFFFFFUL << (22UL - kNumMantissaBits))) +
              // (0x01UL << (22UL - kNumMantissaBits)));
          y = gc->pool.ftemp2 + 0.5F;//((*(FxU32*)&gc->pool.ftemp2 & (0xFFFFFFFFUL << (22UL - kNumMantissaBits))) +
              // (0x01UL << (22UL - kNumMantissaBits)));
          ...
          /* Upper right corner */
          y -= 1.0F;//(0x01UL << (21UL - kNumMantissaBits));
		  x += 0.5F;
          DA_SETF(x);
          dataElem = 0;
          DA_SETF(y);
          ...
          /* Upper Left corner */
          x -= 1;//(0x01UL << (21UL - kNumMantissaBits));
