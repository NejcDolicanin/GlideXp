...
**
** $Header: /cvsroot/glide/glide3x/h5/glide3/src/gtex.c,v 1.3 2000/11/15 23:32:53 joseph Exp $
** $Log: 
...
#ifdef GLIDE2
// Put in GR_LOD and get out GR_LOD_LOG2
GrLOD_t TOG3LODTable[] = {
	GR_LOD_LOG2_2048,	/* NAPALM */
	GR_LOD_LOG2_1024,	/* NAPALM */
	GR_LOD_LOG2_512,	/* NAPALM */
	GR_LOD_LOG2_256,
	GR_LOD_LOG2_128,
	GR_LOD_LOG2_64,
	GR_LOD_LOG2_32,
...
// Put in GR_LOD_LOG2 and get out GR_LOD
GrLOD_t TOG2LODTable[] = {
	GR_LOD_1,
	GR_LOD_2,
	GR_LOD_4,
	GR_LOD_8,
...
// Put in GR_ASPECT and get out GR_ASPECT_LOG2
GrAspectRatio_t TOG3AspectTable[] = {
	GR_ASPECT_LOG2_8x1,	//  GR_ASPECT_8x1 0x0       
	GR_ASPECT_LOG2_4x1,	//  GR_ASPECT_4x1 0x1
	GR_ASPECT_LOG2_2x1,	//  GR_ASPECT_2x1 0x2
	GR_ASPECT_LOG2_1x1,	//  GR_ASPECT_1x1 0x3
	GR_ASPECT_LOG2_1x2,	//  GR_ASPECT_1x2 0x4
	GR_ASPECT_LOG2_1x4,	//  GR_ASPECT_1x4 0x5
	GR_ASPECT_LOG2_1x8	//  GR_ASPECT_1x8 0x6
};

// Put in GR_ASPECT_LOG2 and get out GR_ASPECT	
GrAspectRatio_t TOG2AspectTable[] = {
	GR_ASPECT_1x8,		//  GR_ASPECT_LOG2_1x8       -3
	GR_ASPECT_1x4,		//  GR_ASPECT_LOG2_1x4       -2
	GR_ASPECT_1x2,		//  GR_ASPECT_LOG2_1x2       -1
	GR_ASPECT_1x1,		//  GR_ASPECT_LOG2_1x1        0
	GR_ASPECT_2x1,		//  GR_ASPECT_LOG2_2x1        1
	GR_ASPECT_4x1,		//  GR_ASPECT_LOG2_4x1        2
	GR_ASPECT_8x1		//  GR_ASPECT_LOG2_8x1        3
};

// Put in GR_G2_TEXBASE and get out GR_TEXBASE
GrTexBaseRange_t TOG3TexBaseTable[] = 
{
	GR_TEXBASE_256,
	GR_TEXBASE_128,
	GR_TEXBASE_64,
...
// Put in GR_TEXBASE and get out GR_G2_TEXBASE
GrTexBaseRange_t TOG2TexBaseTable[] = 
{
	GR_G2_TEXBASE_32_TO_1,
	GR_G2_TEXBASE_64,
	GR_G2_TEXBASE_128,
	GR_G2_TEXBASE_256,
};
      ...
      case GR_LOD_LOG2_1:
        {
          ...
          case GR_TEXFMT_ARGB_CMP_DXT1:
            /* For these, we offset the LOD height * texture stride */
            if (evenOdd & GR_MIPMAPLEVELMASK_EVEN) {
              tileY += HEIGHT_BY_ASPECT_LOD_FXT1(aspect, GR_LOD_LOG2_1);
            }
            break;
          case GR_TEXFMT_ARGB_CMP_DXT2:
          case GR_TEXFMT_ARGB_CMP_DXT3:
          ...
          case GR_TEXFMT_ARGB_CMP_DXT5:
            /* For these, we offset the LOD height * texture stride */
            if (evenOdd & GR_MIPMAPLEVELMASK_EVEN) {
              tileY += HEIGHT_BY_ASPECT_LOD_DXT(aspect, GR_LOD_LOG2_1);
            }
            break;
          default:
            /* For these, we offset the LOD height * texture stride */
            if (evenOdd & GR_MIPMAPLEVELMASK_EVEN) {
              tileY += HEIGHT_BY_ASPECT_LOD(aspect, GR_LOD_LOG2_1);
            }
          }
        }
        level++;
...
#define FN_NAME "grTexMipMapMode"
   ...
   * look better.
   *
   * Also, if subsample lod dither is set to 2, we will never allow
   * normal lod dither as well.
   *
   */
  if ( ((gc->sliCount != gc->chipCount) && (_GlideRoot.environment.texSubLodDither == 2)) ||	// Never
	  (_GlideRoot.environment.texLodDither < 0) ) {

	  texMode &= ~SST_TLODDITHER;
  }
  else if (_GlideRoot.environment.texLodDither > 0) {		// Always
...
#ifdef GLIDE2
{
	TO_GLIDE3_LOD(info->largeLodLog2);
	TO_GLIDE3_LOD(info->smallLodLog2);
	TO_GLIDE3_ASPECT(info->aspectRatioLog2);

	_G3grTexSource (tmu, startAddress, evenOdd, info);

	TO_GLIDE2_LOD(info->largeLodLog2);
	TO_GLIDE2_LOD(info->smallLodLog2);
	TO_GLIDE2_ASPECT(info->aspectRatioLog2);
}
#define grTexSource  _G3grTexSource 

void _G3grTexSource (GrChipID_t tmu, FxU32 startAddress, FxU32 evenOdd, GrTexInfo *info)
#endif
{
...
#ifdef GLIDE2
  gc->state.current_mm[tmu] = GR_NULL_MIPMAP_HANDLE;
#endif
...
#ifdef GLIDE2
{
	...
	_G3grTexMultibaseAddress (tmu, AS_GLIDE3_TEXBASE(range), startAddress,
		evenOdd, info);
...
void _G3grTexMultibaseAddress (GrChipID_t tmu, GrTexBaseRange_t range, FxU32 startAddress,
          FxU32 evenOdd, GrTexInfo *info)
#endif
...
/*-------------------------------------------------------------------
  ...
  Return:			Yes
  -------------------------------------------------------------------*/
static FxU32 MultitextureAndTrilinear(void);

void g3LodBiasPerChip(void)
{
	#define FN_NAME "g3LodBiasPerChip"

	FxI32 lodBias, tLod, tmu, tmus;
	int idx = 0;
	unsigned int i;
  	...
  	int chipLodBias[2][4] =       // these values per Gary Tarolli
				...
				//   0.00, 0.50, 0.00, 0.00 - 2 chip and 4 chip 2x
					{0x00, 0x02, 0x00, 0x02},
				//   0.00, 0.25, 0.50, 0.75 - 4 chip, 4x, 8x.
					{0x00, 0x01, 0x02, 0x03}
				};
	...
	tmu = 0;
	idx = (gc->chipCount > 2) - (gc->sliCount > 1);

	// Which tmus can do it?
	...
	// TMU 0
	if (tmus & GR_TMUMASK_TMU0) for (i = 0; i < gc->chipCount; i++)	
	{
	  	tLod = gc->state.tmuShadow[tmu].tLOD;
  		tLod &= ~(SST_LODBIAS);
		lodBias = chipLodBias[idx][i];

		/* Also use envrionment set lodBias */
		lodBias += _GlideRoot.environment.lodBias;
	...
	// TMU 1
	tmu = 1;
	if (tmus & GR_TMUMASK_TMU1) for (i = 0; i < gc->chipCount; i++)	
	{
  	tLod = gc->state.tmuShadow[tmu].tLOD;
  		tLod &= ~(SST_LODBIAS);
		lodBias = chipLodBias[idx][i];
...
/*-------------------------------------------------------------------
  ...
  Return:		FxU32 tmuMask of the TMUs that can do it
  -------------------------------------------------------------------*/
static FxU32 MultitextureAndTrilinear(void)
{
#define FN_NAME "MultitextureAndTrilinear"
	...
	// If we don't have the env variable or if in SLI we can't do it
	if ( (_GlideRoot.environment.texSubLodDither == 0) || (gc->sliCount == gc->chipCount)) {
		return 0;
	}
	
#if SUBSAMPLE_DITHERING_NEEDS_BILINEAR
	
	// Both even and odd on the TMUs and bilinear for mag and min filter,
	// not trilinear and not ditherered lods
	
	if( (gc->state.tmuShadow[0].textureMode & (SST_TMINFILTER | SST_TMAGFILTER) &&
		!(gc->state.tmuShadow[0].textureMode & (SST_TRILINEAR | SST_TLODDITHER)) &&
		(gc->state.per_tmu[0].evenOdd == 3)) {

		retval |= GR_TMUMASK_TMU0;
	}
	
	if( (gc->state.tmuShadow[1].textureMode & (SST_TMINFILTER | SST_TMAGFILTER) &&
		!(gc->state.tmuShadow[1].textureMode & (SST_TRILINEAR | SST_TLODDITHER)) &&
		(gc->state.per_tmu[1].evenOdd == 3)) {

		retval |= GR_TMUMASK_TMU1;
	}
#else
	// Just even and odd and not trilinear and not ditherered lods
	if( !(gc->state.tmuShadow[0].textureMode & (SST_TRILINEAR | SST_TLODDITHER)) &&
		(gc->state.per_tmu[0].evenOdd == 3) ) {
		
		retval |= GR_TMUMASK_TMU0;
	}
	
	// Can't have trilinear here, or on tmu 0
	if( !(gc->state.tmuShadow[1].textureMode & (SST_TRILINEAR | SST_TLODDITHER)) &&
