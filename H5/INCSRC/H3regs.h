...
/* -*-c++-*- */
/* $Header: /cvsroot/glide/glide3x/h5/incsrc/h3regs.h,v 1.3 2000/11/15 23:32:58 joseph Exp $ */
/*
...
typedef volatile struct sstioregs {     
	...
	union {
		struct {
			...
			// Init Registers
			FxU32 status;				// 00 aliased status register
			FxU32 pciInit0;				// 04 
			FxU32 sipMonitor;			// 08 
			FxU32 lfbMemoryConfig;		// 0C This is more or less a dummy variable now.
			...
			FxU32 vgaInit0;				// 28
			FxU32 vgaInit1;				// 2C
			FxU32 dramCommand;			// 30
			FxU32 dramData;				// 34
			FxU32 strapInfo;			// 38
			FxU32 vidTvOutBlankVCount;	// 3C
			...
			// Video Registers I
			FxU32 vidMaxRGBDelta;		// 58 
			FxU32 vidProcCfg;			// 5C
			FxU32 hwCurPatAddr;			// 60
			FxU32 hwCurLoc;				// 64
			FxU32 hwCurC0;				// 68
			FxU32 hwCurC1;				// 6C
			FxU32 vidInFormat;			// 70
			FxU32 vidTvOutBlankHCount;	// 74
			FxU32 vidSerialParallelPort;// 78
			FxU32 vidInXDecimDeltas;	// 7C
			FxU32 vidInDecimInitErrs;	// 80
			FxU32 vidInYDecimDeltas;	// 84
			FxU32 vidPixelBufThold;		// 88
			FxU32 vidChromaMin;			// 8C
			FxU32 vidChromaMax;			// 90
			FxU32 vidCurrentLine;		// 94
			FxU32 vidScreenSize;		// 98
			FxU32 vidOverlayStartCoords;// 9C
			FxU32 vidOverlayEndScreenCoord;// A0
			FxU32 vidOverlayDudx;		// A4
			FxU32 vidOverlayDudxOffsetSrcWidth;//A8
			FxU32 vidOverlayDvdy;		// AC
			
			// VGA Registers
			FxU32 vgaRegister[12];		// B0->DC
			
			// Video Registers II
			FxU32 vidOverlayDvdyOffset;	// E0
			FxU32 vidDesktopStartAddr;	// E4
			FxU32 vidDesktopOverlayStride;// E8
			FxU32 vidInAddr0;			// EC
			FxU32 vidInAddr1;			// F0
			FxU32 vidInAddr2;			// F4
			FxU32 vidInStride;			// F8
			FxU32 vidCurrOverlayStartAddr;// FC
			
			// The following registers don't exist in IO space where
			// their offset is in this structure.
			FxU32 lfbMemoryTileCtrl;	// 104 = 256
			FxU32 lfbMemoryTileCompare;	// 108 = 260
		};
